...
The tool shall optionally call the GitHub Copilot SDK to score each prompt/response pair (0–100) and surface prompts scoring below a configurable threshold (default 60). Analysis runs sequentially to respect SDK rate limits.
...
Files with `.md` extension shall be rendered with full markdown formatting: headings (H1–H3), **bold**, *italic*, `` `code` ``, fenced code blocks, ordered/unordered lists, blockquotes, and horizontal rules.
