...
export function Header({ runId, status = 'idle', version = '0.2.1', mode = 'analysis', projectRoot, provider = 'copilot' }: HeaderProps) {
  ...
  const providerLabel = provider === 'claude' ? '◆ claude' : '◆ copilot';
  const providerColor = provider === 'claude' ? 'magenta' : 'green';
